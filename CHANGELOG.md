...
- **Message navigation now survives thread switches.** Each conversation retains its message-index entries and active position locally, restores them immediately when revisited, and still accepts the host's asynchronous refresh as the source of truth.
