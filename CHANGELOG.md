...
- **Message navigation now survives thread switches.** Each conversation retains its message-index entries and active position locally, rebuilds them from restored transcript data when revisited, and refreshes the full index after completed runs. Hover previews are fully opaque for clearer reading over transcript content, while the surrounding navigation remains subdued.
