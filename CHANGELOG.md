...
- **Expanded tool details use the available width.** Their left edge now aligns with the tool icon instead of the title, giving arguments and output more room.
- **Tool timelines now survive thread switches.** Bounded tool arguments, results, status, and perceived timing are stored in the local conversation log and reconstructed when a thread is reopened.
- **Tool runs are now concise, inspectable timelines.** Live tool calls remain expanded while work is in progress, then collapse into an English summary when the agent replies. The summary shows tool count and user-perceived elapsed time—from the prompt through model, network, scheduling, and tool latency until the response begins; expand it for per-tool purpose, status, duration, arguments, output/error, and file access. Common read, search, command, edit, Context Mode, skill, and web tools have dedicated presentation, while new tools receive a useful fallback automatically.
- **Jump between your messages without losing your place.** A near-invisible, edge-hover transcript navigator stacks one evenly spaced mark for every user query and grows naturally with the conversation. At rest all marks are equal length and only the current query is highlighted; hovering makes the pointed mark longest and tapers its neighbours, while seamless hit areas, a fully opaque readable preview, and a brief landing animation keep navigation easy to follow.
...
- **Split Context Mode code execution out of the “Run commands” auto-approve.** `ctx_execute`, `ctx_execute_file`, `ctx_batch_execute`, `ctx_purge` and `ctx_upgrade` used to ride along with shell `execute`, so approving ordinary commands also approved arbitrary unsandboxed code. They now have their own **Context Mode code (ctx_execute)** checkbox, off by default; **Run commands** stays on and covers shell only. Existing saved states pick up the new category as off. Allowing full access still turns both on, matching what that mode actually permits.
...
- **Clinical sample rows is now an approval, not a preference.** The checkbox left the Settings panel and joined the composer **Auto-approve** list next to Full access, and it is off by default — the agent refuses patient ID / MRN / `OR_LOG` ID until you opt in. It also moved out of `.clawagents/vscode_settings.json` into workspace-scoped VS Code SecretStorage, alongside Full access and the gateway/MCP grants. That closes a real hole: the settings file lives in the repository, so a cloned or shared workspace could previously ship `allow_clinical_samples: true` and authorize pasting identifiers with no prompt. A committed value is now ignored outright and dropped from the file on the next save, which also clears the stale `true` that 1.0.152 wrote into every workspace by default.
- **The network pip bootstrap asks first, and validates what it downloads.** `get-pip.py` runs code fetched over the network, so it is now gated behind an explicit modal — the same treatment `pip --break-system-packages` already gets — and never runs unattended. The fetch is HTTPS-only with certificate validation, refuses redirects off `bootstrap.pypa.io`, caps the response at 16 MB, and rejects a body that is not a Python script, so a captive portal or proxy error page cannot be executed.
- **Stop repeating the same startup failure on every message.** Each webview action calls `ensureStarted()`, so a host that could not build the environment rebuilt and re-deleted it once per send — which is why the same wall of venv text appeared over and over. A failed build is now remembered for 30 seconds and replayed, and **ClawAgents: Restart Sidecar** clears it immediately so a host you just fixed retries at once.
