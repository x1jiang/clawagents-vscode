...
Publisher **`clawagents`** · source https://github.com/x1jiang/clawagents-vscode · version **1.0.43**
...
- [x] **Publish `clawagents` 6.15.0 to PyPI first** (1.0.43 requires `>=6.15.0`)
- [x] VSIX built: `clawagents-1.0.43.vsix`
...
VSCE_PAT='paste-token-here' npx @vscode/vsce publish --packagePath ./clawagents-1.0.43.vsix
...
```bash
npx ovsx publish ./clawagents-1.0.43.vsix -p "$OVSX_PAT"
```
