...
class TestMergeCuratedWithRemote(unittest.TestCase):
    ...
    def test_falls_back_to_remote_when_no_overlap(self) -> None:
        curated = [{"id": "gpt-5.6-luna", "label": "GPT-5.6 Luna"}]
        remote = [{"id": "davinci-002", "label": "davinci-002"}]
        out = providers._merge_curated_with_remote(curated, remote)
        self.assertEqual(out, remote)

    def test_falls_back_to_curated_when_remote_empty(self) -> None:
        curated = [{"id": "gpt-5.6-luna", "label": "GPT-5.6 Luna"}]
        out = providers._merge_curated_with_remote(curated, [])
